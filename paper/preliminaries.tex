...
of the lattice problem and is the main lever \textsc{Kyber} uses to raise its claimed security level~\cite{kyberround1}. Increasing the noise
variance also increases the estimated attack cost, which is why \textsc{Kaiburr}
adjusts the noise distribution as $k$ increases in this work.
...
distribution beyond $q/4$, scaled by $n$. NIST requires this probability to be bounded by $2^{-128}$ for ML-KEM's target security
level~\cite{fips203}; we use the same bound throughout as our correctness target.
