...
implementation for Pavona's Asymmetric Cryptography Coprocessor (ACC). The ACC reuses ML-KEM's 
...
These changes nonetheless affect more code than they touch directly. ML-KEM
...
Larger values of $k$ can overflow the matrix-vector product. In the NTT
domain, each polynomial of the product is a sum of $k$ pointwise products.
ML-KEM accumulates this sum in an \texttt{int16\_t} and reduces it once at the
end. Each Montgomery product has magnitude below $q$, so the sum fits whenever
$kq < 2^{15}$, i.e.\ $k \le 9$. This holds for the standardized $k \le 4$, but
fails for our large parameters $k=18$ or $k=24$.

All three implementations therefore reduce the accumulator after every
pointwise product. We apply this in all cases, including for $k=7$, where it is
not strictly necessary. The cost is one extra reduction per term i.e. a Barrett
reduction in the C and Jasmin code, and a modular addition against the
\texttt{MOD} register on ACC.
...
vectorized and assembly variant, the sign is then applied separately and
without branching.

Extracting $t$-bit fields from the PRF output is simplest for $t=8$ and $t=4$,
which align with bytes and nibbles, and hardest for $t=6$. Both AVX2 kernels
use an additional shuffle or gather to align the $6$-bit fields. The C kernel
reads past the end of its input, so the PRF buffer carries $32$ bytes of
trailing slack. ACC has no byte-shuffle instruction, so it instead chains
\texttt{bn.rshi} shifts to move a $6$-bit window across register boundaries.
...
implementation is validated against a Python model, which subclasses
\texttt{kyber-py}'s \texttt{ML\_KEM}, overrides the noise sampler, and
