...
to it for long-term confidentiality \cite{frodokem-news}; it was also recently standardized by ISO \cite{frodoiso}. HQC, the second 
KEM standardized by NIST in the fourth round, provides a different point of comparison with its security relying
 on the hardness of the Quasi-Cyclic Syndrome Decoding (QCSD) problem rather than LWE. While HQC provides cryptographic diversity and was 
 chosen as a code-based backup to complement ML-KEM, FrodoKEM is understood to be the conservative choice within the lattice-based setting.
...
hypothetical structural attack, while removing structure can affect the cost of known attacks. Introducing a new hardness assumption, such as HQC's QCSD 
instead of LWE, provides cryptographic diversity, since an attack on one assumption does not automatically imply an attack on the other. 
However, diversity alone also does not imply complete cryptanalytic independence.
...
Moreover, the quasi-cyclic structure of HQC has been noted to have important parallels with the ring and module structures used in structured lattice schemes \cite{hqc-forum}. 
These approaches address different sources of uncertainty, and it is impossible to predict which is more likely to matter.
...
today's KEMs carry enough but how much margin they can carry at all. The second question arises because additional margin comes at a cost, 
