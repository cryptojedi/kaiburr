...
to it for long-term confidentiality \cite{frodokem-news}; it was also recently standardized by ISO \cite{frodoiso}. Outside the lattice setting, 
NIST's fourth round also standardized HQC, whose security rests on the Quasi-Cyclic Syndrome Decoding (QCSD) problem rather than LWE \cite{hqc2025spec}. 
HQC was chosen as a code-based backup that provides cryptographic diversity alongside ML-KEM, whereas FrodoKEM is understood to be the conservative choice 
within the lattice-based setting.
...
\begin{itemize}
  \item \emph{We show that naively scaling ML-KEM's module rank hits a correctness wall.} Raising $k$ alone, with Kyber's noise and 
  ...
  \item \emph{We introduce a one-parameter noise family that allows us to scale further.} Our family $f(t)$
    generalizes Kyber's centered binomial distribution, thinning its $\pm 2$ tail geometrically as $t$
    grows while leaving the $\pm 1$ mass and its branchless sampling unchanged.
    This gives a noise knob that can be traded against module rank to keep decryption failure below
  ...
  \item \emph{We select concrete parameter sets that reach large security margins within FrodoKEM-640's
    ...
    bits while their public keys and ciphertexts stay within within the communication budget of FrodoKEM-640, its smallest standardized
parameter set (see Section~\ref{sec:benchmarks}).
  \item \emph{We build three implementations of \textsc{Kaiburr}.} We implement all three
  ...
  \item \emph{We benchmark \textsc{Kaiburr} against FrodoKEM and HQC.} Across three Intel
    microarchitectures, our AVX2 and Jasmin implementations run substantially faster than FrodoKEM-640 and 
