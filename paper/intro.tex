...
for standardization in the fourth round~\cite{fourthstatus}. Unlike ML-KEM, which is based on module learning with errors (LWE), 
HQC relies on the Quasi-Cyclic Syndrome Decoding (QCSD) 
...
is based on a more conservative hardness assumption of plain LWE, one of the reasons why it was
recommended by bodies such as BSI, ENISA, and NLNCSA~\cite{frodokem-news} and recently standardized by ISO~\cite{frodoiso}.

It is useful to separate the two notions of conservatism this framing bundles together as they concern two different
classes of attack. The first arises from the choice of hardness assumption. 
...
hedges are distinct, but not entirely independent because additional parameter margin can also increase the cost of a 
hypothetical structural attack, and removing algebraic structure can affect the cost of known attacks. A third, orthogonal hedge is to change the hardness 
assumption altogether, as HQC does by relying on QCSD instead of LWE. This provides diversity, and at first glance, some additional assurance
since a break of LWE could still leave HQC secure. However, this cryptanalytic independence is not absolute.
...
The quasi-cyclic structure of HQC has also been noted to have important parallels with the ring and module structures used in structured lattice schemes~\cite{hqc-forum}.
These approaches address different (hypothetical) ways things could go wrong: unstructured LWE protects against attacks that exploit algebraic structure, 
additional parameter margin protects against improvements to existing attacks, and QCSD provides diversity against a failure of the LWE assumption itself.
At this time, it is impossible to predict which one of these breakthroughs is more likely to occur.
...
adjusts the noise distribution to achieve substantially higher security estimates for much larger ciphertexts. This leaves open the corresponding question for module-based KEMs:

\emph{How much additional security margin can their algebraic structure buy within the communication budget of an unstructured LWE scheme, and at what computational cost?}
...
\begin{itemize}
  ...
  makes this bound provable rather than heuristic and close a gap that exists for ML-KEM's published bound by removing compression 
...
\makeatletter
\if@submission
All software, including parameter analysis scripts, implementation source code, and other artifacts, is 
included in the artifact submitted alongside this paper. It will be publicly available under a permissive license.
\else
