...
$q=3329$ is a suitable small prime satisfying the NTT-enabling condition $n \mid (q-1)$, notably weaker than the $2n \mid (q-1)$ one might expect, 
...
and results in a different scheme altogether. We choose not to change $q$ for this reason.

This is not to say that $k$ is the only parameter that can be modified. We focus on changes that preserve the \emph{essence} of \textsc{Kyber}:
...
Since the noise only affects the secret and error terms and does not interact with the arithmetic structure or the NTT, 
...
In this sense, the error distribution is another part of the scheme alongside $k$ that moves both security and correctness without changing what 
\textsc{Kyber} is at its core. This is the direction we pursue and where our main contribution lies.
...
\[
  ...
  \Pr[0] = \tfrac12 - 2^{-(t-1)}.
\]
The mass at $\pm 1$ is fixed. The single parameter $t$ acts only on the
...
Crucially, $f(t)$ generalizes \textsc{Kyber}'s own noise distribution. At $t=4$, the probabilities become
$$
...
\subsection{Finer control over the noise}
...
$f(t)$ ranges from $\mathrm{CBD}_2$ toward $\mathrm{CBD}_1$ as $t$ increases.
...
values are natural, especially for AVX2 implementations (see Section~\ref{sec:implementation}). As shown above, $t=4$ recovers the $\mathrm{CBD}_2$ distribution, so it serves
...
bytes and 9,720 ciphertext bytes. This gives us 
...
\begin{table}[H]
  \centering
  \caption{\textsc{Kaiburr} parameter sets. Classical/quantum security (Core-SVP) and failure probability computed with the \textsc{Kyber} team's scripts; other parameters as in ML-KEM, compression removed.}
...
\subsection{Sizes in context}
Table~\ref{tab:size-comparison} places the three \textsc{Kaiburr} parameter
sets alongside all standardized FrodoKEM and HQC parameter sets. All three \textsc{Kaiburr} variants, including kaiburr-6144, remain within the
communication budget of FrodoKEM-640, the smallest FrodoKEM parameter set,
despite reaching Core-SVP security levels far beyond the NIST Level 5 target. kaiburr-1792 is smaller in
combined pk+ct size than HQC-1 (5,792 vs.\ 6,674 bytes), and less than
a third the size of HQC-5 (21,658 bytes)

\begin{table}[h]
\centering
\caption{Public-key and ciphertext sizes across \textsc{Kaiburr},
FrodoKEM~\cite{frodoKEM-github}, and HQC~\cite{hqc-github,hqc2025spec}.
\textsc{Kaiburr}'s classical/quantum Core-SVP security is as computed with
the \textsc{Kyber} team's scripts (Table~\ref{tab:kaiburr-params}); FrodoKEM
and HQC security categories are as claimed by their designers against NIST's
category 1/3/5 targets and are not directly comparable to the Core-SVP
numbers.}
\label{tab:size-comparison}
\begin{tabular}{lcccc}
\toprule
Scheme & Security & pk (bytes) & ct (bytes) & pk+ct (bytes) \\
\midrule
kaiburr-1792 & 479 / 434 (class./quant.) & 2720  & 3072  & 5792  \\
kaiburr-4608 & 1281 / 1162 (class./quant.) & 6944  & 7296  & 14240 \\
kaiburr-6144 & 1717 / 1557 (class./quant.) & 9248  & 9600  & 18848 \\
\midrule
FrodoKEM-640  & Category 1 & 9616  & 9720  & 19336 \\
FrodoKEM-976  & Category 3 & 15632 & 15744 & 31376 \\
FrodoKEM-1344 & Category 5 & 21520 & 21632 & 43152 \\
\midrule
HQC-1 & Category 1 & 2241 & 4433  & 6674  \\
HQC-3 & Category 3 & 4514 & 8978  & 13492 \\
HQC-5 & Category 5 & 7237 & 14421 & 21658 \\
\bottomrule
\end{tabular}
\end{table}
